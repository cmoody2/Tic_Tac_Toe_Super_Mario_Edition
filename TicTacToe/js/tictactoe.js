...
let selectedSquares = [];       //this array stores an array of moves. We use this to determine win conditiions.
...
function placeXOrO(squareNumber) {                                                  //this function is for placing an x or o in a square.
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        let select = document.getElementById(squareNumber);                         //this variable retireves the html element id that was clicked.                                  
        ...
        audio('./media/smb_coin.wav');                                                 //this function plays placement sound.                                             
...
function checkWinConditions() {                                                         
    ...
    else if (arrayIncludes('6X', '7X', '8X')) { drawWinLine(50, 508, 558, 508); }         //X 6, 7, 8 condtion
    else if (arrayIncludes('0X', '3X', '6X')) { drawWinLine(100, 50, 100, 558); }       //X 0, 3, 6 condition
    else if (arrayIncludes('1X', '4X', '7X')) { drawWinLine(304, 50, 304, 558); }         //X 1, 4, 7 conditiion
    else if (arrayIncludes('2X', '5X', '8X')) { drawWinLine(508, 50, 508, 558); }       //X 2, 5, 8 condition
    else if (arrayIncludes('6X', '4X', '2X')) { drawWinLine(100, 508, 510, 90); }         //X 6, 4, 2 conditiion
    ...
    //and 9 squares are selected, the code executes.
        ...
        audio('./media/smb_pause.wav');                                                   //this function plays the tie game sound.
        setTimeout(function () { resetGame(); }, 1000);                                //this function sets a 1 second timer before the resetGame is called.
    }

    //this function checks if an array includes 3 strings.
    //it is used to check for each win condition
    function arrayIncludes(sqaureA, squareB, squareC) {                             //the next 3 variables will be used to check for 3 in a row.          
        const a = selectedSquares.includes(sqaureA);
        const b = selectedSquares.includes(squareB);
        const c = selectedSquares.includes(squareC);
        if (a === true && b === true && c === true) {return true; }                 //if the 3 variables we pass are all included in our array true is returned 
...
function audio(audioURL) {                                                      //we create a new audio object and we pass the path as a parameter                                             
    let audio = new Audio(audioURL);                                                //play method plays our audio sound.
    audio.play();
}
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {                      //this function utilizes html canvas to draw win lines.
    ...
    let x1 = coordX1,                                                               //this line indicates where the start of a lines x axis is.
        ...
        y = y1;                                                                 //this variable storees temporary y axis date we update in our animation loop.
    ...
    function animateLineDrawing() {                                                 //this function interacts with the canvas
        ...
        c.strokeStyle = 'rgba(70, 255, 33, .8)';                                        //this method draws everything we laid out above
        ...
        //if was necessary for the 6, 4, 2 win condtion
        if (x1 <= x2 && y1 >= y2) {                                                 
            if (x < x2) { x += 10; }                                
            if (y > y2) { y -= 10; }
    ...
    function clear() {                                                   //this function clear our canvas after our win line is drawn
    ...
    audio('./media/smb_powerup.wav');                                           //this line plays win sounds
